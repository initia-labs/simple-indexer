...
export class RPCSocket {
  ...
  protected onError(error): void {
    console.log(`${this.constructor.name} websocket: `, error)
  }
  ...
  protected async onRawData(raw): Promise<void> {
    ...
    try {
      ...
      console.log(`${this.constructor.name}: JSON parse error ${raw}`)
      return
    }
...
export class RPCClient {
  ...
  constructor(public rpcUrls: string[]) {
    if (this.rpcUrls.length === 0) {
      throw new Error('RPC URLs list cannot be empty')
    }
    this.curRPCUrlIndex = 0
  ...
  public rotateRPC() {
    this.curRPCUrlIndex = (this.curRPCUrlIndex + 1) % this.rpcUrls.length
    this.rpcUrl = this.rpcUrls[this.curRPCUrlIndex]
    console.log(`Rotate RPC to ${this.rpcUrl}`)
  }
  ...
  async getBlockchain(
    ...
    if (!blockchainResult) {
      console.log('failed get blockchain from rpc')
      return null
    }
  ...
  async getBlockBulk(start: string, end: string): Promise<BlockBulk | null> {
    ...
    if (!blockBulksResult) {
      console.log('failed get block bulks from rpc')
      return null
    }
